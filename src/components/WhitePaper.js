...
function WhitePaper(props) {
  ...
  const onScrollHandler = (e) => {
    let el = window;
    const widthNHeightObj = {x: el.pageXOffset !== undefined ? el.pageXOffset : el.scrollLeft,
    y: el.pageYOffset !== undefined ? el.pageYOffset : el.scrollTop};
    //console.log("widthNHeightObj ===>",widthNHeightObj);

    let whichPageIamIn = Math.ceil(widthNHeightObj.y/document.body.clientHeight);
    
    //console.log("786 ===>",whichPageIamIn);
    pageNo!=whichPageIamIn ? (setPageNo(whichPageIamIn),setAnimateClass('targetAnimation')) : '';
  }
